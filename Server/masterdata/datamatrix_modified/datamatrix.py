...
class DataMatrix:
    ...
    def __init__(self, msg: str, rect: bool = False, codecs: list[str] = ["ascii", "C40", "text", "X12", "edifact"]):
        self.message = msg
    ...
    def svg(self, fg="#000", bg="#FFF"):
        ...
        height = len(mat) + 2
        width = len(mat[0]) + 2
        return svg_template.format(fg=fg, bg=bg, path_cmds=cmds, height=height, width=width)
    ...
    def matrix(self):
        ...
        M = {}
        enc1: list[tuple[str, bytes]] = []
        for codec in self.codecs:
            try:
                enc1.append((codec, self.message.encode(codec)))
            except ValueError:
                # This message is not encodable in this codec. Skip it.
                pass
        enc2 = min(enc1, key=lambda x: len(x[1]))
        self.usedCodec = enc2[0]
        enc = {i: c for i, c in enumerate(enc2[1])}
        el = len(enc)
        ...
        while True:
            ...
            if r == h - 3 and c == -1:
                k = [w, 6 - h, w, 5 - h, w, 4 - h, w, 3 - h, w - 1, 3 - h, 3, 2, 2, 2, 1, 2]  # corner A layout
            elif r == h + 1 and c == 1 and (w & 7) == 0 and (h & 7) == 6:
                k = [w - 2, -h, w - 3, -h, w - 4, -h, w - 2, -1 - h, w - 3, -1 - h, w - 4, -1 - h, w - 2, -2, -1, -2]  # corner D layout
            else:
                ...
                if r == h - 2 and c == 0 and (w & 3):
                    k = [w - 1, 3 - h, w - 1, 2 - h, w - 2, 2 - h, w - 3, 2 - h, w - 4, 2 - h, 0, 1, 0, 0, 0, -1]  # corner B layout
                elif r == h - 2 and c == 0 and (w & 7) == 4:
                    k = [w - 1, 5 - h, w - 1, 4 - h, w - 1, 3 - h, w - 1, 2 - h, w - 2, 2 - h, 0, 1, 0, 0, 0, -1]  # corner C layout
