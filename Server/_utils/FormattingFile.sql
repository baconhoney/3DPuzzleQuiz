...
SELECT buildings.name_{lang}, buildings.country_{lang}, buildings.city_{lang}, answers.answer, CASE WHEN buildings.answer = answers.answer THEN 1 ELSE 0 END
FROM buildings JOIN quizzes ON buildings.id = quizzes.building_id LEFT JOIN answers ON buildings.id = answers.building_id
WHERE quizzes.quiz_number = {quizNumber} AND answers.team_id = {teamID}
